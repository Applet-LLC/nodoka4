					   ...
					   int nCmdShow)
{
	...
	// WinVerifyTrust check for hook DLLs before loading.
	{
		...
		GetModuleFileName(hInst, modPath, MAX_PATH);
		// pathRemoveFileSpec equivalent
		TCHAR *last = _tcsrchr(modPath, _T('\\'));
		if (last) *last = _T('\0');
		tstring dir(modPath);
		bool hookOK = verifyDllSignature(dir + _T("\\nodoka.dll"))
		           && verifyDllSignature(dir + _T("\\sirius_hook_for_nodoka_x86.dll"));
		if (!hookOK)
		{
			OutputDebugString(_T("[nodoka_helper][WARNING] Hook DLL signature verification failed - degraded operation is in progress\n"));
			// Skip hook installation to avoid loading unverified DLLs
			goto skip_hooks;
		}
	}
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
	...
	hWnd = CreateWindowEx(0, szWindowClass, szTitle, WS_OVERLAPPEDWINDOW | WS_DISABLED,
