...
{
#ifdef FOR_LIMIT
	m_actions.pop_back(); // KeySeqには1個しか保存できない。
#endif
...
tostream &operator<<(tostream &i_ost, const KeySeq &i_ks)
{
	for (KeySeq::Actions::const_iterator
			 i = i_ks.m_actions.begin();
		 i != i_ks.m_actions.end(); ++i)
		i_ost << **i << _T(" ");
	return i_ost;
}
							...
							const tstringi &i_titleName)
{
	...
	if (boost::regex_search(i_className, m_windowClass))
	{
		if (m_type == Type_windowAnd)
			return boost::regex_search(i_titleName, m_windowTitle);
...
// Example: "S-" rule has 1 care bit; all-dontcare rule has 0 → "S-" wins when Shift is held.
static int countModifierCareBits(const Modifier &m)
{
	int count = 0;
	for (int t = Modifier::Type_begin; t != Modifier::Type_end; ++t)
...
Keymap *Keymaps::searchByName(const tstringi &i_name)
{
	for (KeymapList::iterator
			 i = m_keymapList.begin();
		 i != m_keymapList.end(); ++i)
		if ((*i).getName() == i_name)
			return &*i;
	return NULL;
}
...
KeySeq *KeySeqs::searchByName(const tstringi &i_name)
{
	for (KeySeqList::iterator
			 i = m_keySeqList.begin();
		 i != m_keySeqList.end(); ++i)
		if ((*i).getName() == i_name)
			return &(*i);
	return NULL;
}
