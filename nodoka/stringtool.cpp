	...
	for (const _TCHAR *s = i_data.c_str(); *s; ++s)
	{
		...
		default:
			...
			else
			{
				...
				_TCHAR buf[5];
#ifdef _UNICODE
				_sntprintf_s(buf, NUMBER_OF(buf), _TRUNCATE, _T("%04x"), *s);
#else
				_sntprintf_s(buf, NUMBER_OF(buf), _TRUNCATE, _T("%02x"), *s);
#endif
				i_ost << buf;
			}
			break;
...
tstring addSessionId(const _TCHAR *i_str)
{
	...
	{
		_TCHAR buf[20];
		_sntprintf_s(buf, NUMBER_OF(buf), _TRUNCATE, _T("%u"), sessionId);
		s += buf;
	}
	return s;
}
...
{
	size_t size;
	mbstowcs_s(&size, NULL, NULL, i_str.c_str(), i_str.size());
	if (size == (size_t)-1)
		return std::wstring();
	Array<wchar_t> result(size + 1);
	mbstowcs_s(NULL, result.get(), size + 1, i_str.c_str(), i_str.size());
...
{
	size_t size;
	wcstombs_s(&size, NULL, NULL, i_str.c_str(), i_str.size());
	if (size == (size_t)-1)
		return std::string();
	Array<char> result(size + 1);
	wcstombs_s(NULL, result.get(), size + 1, i_str.c_str(), i_str.size());
