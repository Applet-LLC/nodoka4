		...
		for (int j = 0; j < 4; ++j)
		{
			...
			case ORIGIN_LEFT_EDGE:
				*pos[j].m_out = pos[j].m_pos;
				break;
			case ORIGIN_CENTER:
				*pos[j].m_out = pos[j].m_curWidth / 2 - (pos[j].m_width / 2 - pos[j].m_pos);
				break;
			case ORIGIN_RIGHT_EDGE:
				*pos[j].m_out = pos[j].m_curWidth - (pos[j].m_width - pos[j].m_pos);
				break;
			}
		}
...
// forward message
BOOL LayoutManager::defaultWMHandler(UINT i_message, WPARAM i_wParam, LPARAM i_lParam)
{
	switch (i_message)
	{
	case WM_SIZE:
