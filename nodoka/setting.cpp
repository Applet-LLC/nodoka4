	...
	else if (*t == _T("UnitID"))
	{
		...
		if (m_setting->m_KEY_first == 2)
		{
			...
			if (i_hwnd != NULL) // のどかの通知領域アイコンをTargetにする。
			{
				...
				if (j > 0 && myRegisterRawInputDevices != NULL) // キーボードが一つ以上ある場合登録する。
				{
...
bool readFile(tstring *o_data, const tstringi &i_filename)
{
	...
	{
		DWORD dwLow, dwHigh;
		dwLow = GetCompressedFileSize(i_filename.c_str(), &dwHigh);
		if (GetLastError() == 0)
	...
	size_t wsize;
	mbstowcs_s(&wsize, NULL, NULL, reinterpret_cast<char *>(buf.get()), 0);
	if (wsize != size_t(0))
	{
		Array<wchar_t> wbuf(wsize);
		mbstowcs_s(NULL, wbuf.get(), wsize, reinterpret_cast<char *>(buf.get()), _TRUNCATE);
