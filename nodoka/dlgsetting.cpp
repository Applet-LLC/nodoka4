		...
		case IDC_BUTTON_editfile:
		{
			...
			if (bFlag == TRUE)
			{
				...
				if (CheckVirtualStore(data.m_filename))
				{
					...
					if (bGUI == false)
					{
						errnum = (INT_PTR)ShellExecute(NULL, NULL, data.m_filename.c_str(), NULL, NULL, SW_SHOWNORMAL);
					...
					else
					{
						...
						errnum = (INT_PTR)ShellExecute(NULL, NULL, buff_guiname, name, NULL, SW_SHOWNORMAL);
					}
				}
			}
			return TRUE;
