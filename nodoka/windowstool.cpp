...
// GetWindowRect WA
bool myGetWindowRect(HWND i_hwnd, RECT *o_rc)
{
...
{
	//if (!DwmGetWindowAttribute( i_hwnd, 0x9 /* DWMWA_EXTENDED_FRAME_BOUNDS */, o_rc, sizeof (RECT) ))
...
{
	DWORD dwForegroundID = GetWindowThreadProcessId(GetForegroundWindow(), NULL);
	DWORD dwTargetID = GetWindowThreadProcessId(i_hwnd, NULL);
	DWORD dwsptime;

	if (dwForegroundID == dwTargetID) // 同じだったので、SetForegroundWindowを実行して終了。
	{
...
SetLayeredWindowAttributes_t setLayeredWindowAttributes = initalizeLayerdWindow;
...
(HWND hwnd, DWORD dwFlags) = initializeMonitorFromWindow;
...
static BOOL WINAPI initializeGetMonitorInfo(HMONITOR hMonitor, LPMONITORINFO lpmi)
{
	HMODULE hModule = GetModuleHandle(_T("user32.dll"));
	if (!hModule)
		return FALSE;
...
(HMONITOR hMonitor, LPMONITORINFO lpmi) = initializeGetMonitorInfo;
	...
	HDC hdc, LPRECT lprcClip, MONITORENUMPROC lpfnEnum, LPARAM dwData)
{
	...
	if (proc)
		enumDisplayMonitors =
			reinterpret_cast<BOOL(WINAPI *)(HDC, LPRECT, MONITORENUMPROC, LPARAM)>(proc);
	else
...
WTSRegisterSessionNotification_t wtsRegisterSessionNotification = initializeWTSRegisterSessionNotification;
...
WTSUnRegisterSessionNotification_t wtsUnRegisterSessionNotification = initializeWTSUnRegisterSessionNotification;
...
WTSGetActiveConsoleSessionId_t wtsGetActiveConsoleSessionId = initializeWTSGetActiveConsoleSessionId;
...
{
	size_t size = 0;
	_wgetenv_s(&size, NULL, 0, varname);
	if (size > 0)
	{
		wchar_t *buffer = (wchar_t *)malloc(size * sizeof(wchar_t));
		if (buffer)
		{
			_wgetenv_s(&size, buffer, size, varname);
			return buffer;
		}
	}
	return NULL;
...
LPFN_ISWOW64MESSAGE
fnIsWow64Message = (LPFN_ISWOW64MESSAGE)GetProcAddress(GetModuleHandle(L"user32"), "IsWow64Message");
